    ...
    <div className="flex flex-col h-full bg-slate-50 border-r">
      ...
      <div className="flex items-center justify-between p-4 bg-white border-b shadow-sm z-10">
        <div className="flex items-center gap-3">
          <img src={assets.arrow_icon} onClick={() => setSelectedUser(null)} className="w-6 cursor-pointer md:hidden rotate-180" alt="Back" />
          <img src={selectedUser.profilePic || assets.avatar_icon} className="w-10 h-10 rounded-full object-cover" alt="Avatar" />
          <div>
            ...
            <div key={index} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[70%] flex gap-2 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
                  <div className={`p-3 rounded-2xl ${isOwn ? 'bg-violet-600 text-white rounded-br-none' : 'bg-white border text-gray-800 rounded-bl-none shadow-sm'}`}>
                    {msg.image && <img src={msg.image} alt="Attachment" className="max-w-[200px] rounded-lg mb-2" />}
      ...
      <div className="p-4 bg-white border-t">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3 bg-gray-100 p-2 px-4 rounded-full">
          ...
          <input type="file" id="image-upload" accept="image/png, image/jpg, image/jpeg" hidden onChange={handleSendImage} />
          
          <button type="submit" className="bg-violet-600 p-2 rounded-full hover:bg-violet-700 transition flex-shrink-0">
